...
S3_URL=$(whiptail --inputbox "Please Enter MINIO URL 🌐 (ex:- http://minioip/fqdn:9000) " --title " 🌐 S3 URL 🌐" 10 65 3>&1 1>&2 2>&3 )
S3_BUCKET=$(whiptail --inputbox " Please Enter MINIO Bucket Name 🧺 " --title " 🧺 S3 BUCKET NAME 🧺 " 10 65 3>&1 1>&2 2>&3 )
S3_ACCESS_KEY=$(whiptail --passwordbox " Please Enter MINIO Acess Key 🗝️ " --title " 🗝️ S3 Acess Key 🗝️ " 10 65 3>&1 1>&2 2>&3 )
S3_SECRET_KEY=$(whiptail --passwordbox " Please Enter MINIO SECRET Key 🔐 " --title " 🔐 S3 SECRET KEY 🔐" 10 65 3>&1 1>&2 2>&3 )
...
echo "enable_cinder_backup: "yes"" >> globals.yml
echo "nova_compute_virt_type: "$VIRT_TYPE"" >> globals.yml
echo "s3_url: "$S3_URL"">> globals.yml
echo "s3_bucket: "$S3_BUCKET"" >> globals.yml
echo "s3_access_key: "$S3_ACCESS_KEY"" >> globals.yml
echo "s3_secret_key: "$S3_SECRET_KEY"" >> globals.yml
echo "cinder_backup_driver: "s3"" >> globals.yml
echo "cinder_backup_s3_url: "$S3_URL"" >> globals.yml
echo "cinder_backup_s3_bucket: "$S3_BUCKET"" >> globals.yml
echo "cinder_backup_s3_access_key: "$S3_ACCESS_KEY"" >> globals.yml
echo "cinder_backup_s3_secret_key: "$S3_SECRET_KEY"" >> globals.yml
